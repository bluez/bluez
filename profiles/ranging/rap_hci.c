...
static const struct cs_callback_map_t cs_callback_map[] = {
				    ...
				    void *user_data)
{
	 ...
	 * which is different from the role_enable value sent in
	 * HCI Command
	 */
	...
	for (i = 0; i < CS_CALLBACK_MAP_SIZE; i++) {
			...
			int8_t max_tx_power)
{
	...
	/* Check if state machine already exists for this RAP instance */
	sm = bt_rap_get_hci_sm(rap);
	if (sm) {
		error("State machine already exists for this RAP instance");
		return false;
	}
	...
	/* Store state machine in RAP instance */
	bt_rap_set_hci_sm(rap, sm);
...
void bt_rap_detach_hci(struct bt_rap *rap)
{
	...
	/* Get and cleanup the per-instance state machine */
	sm = bt_rap_get_hci_sm(rap);
	if (sm) {
		...
		/* Clear the reference in RAP instance */
		bt_rap_set_hci_sm(rap, NULL);
	}
